...
import ChoosNFt from "./ChoosNFt";
 
 

const MultiStakeCard = (props) => {
...
//   alert(_array.length);
    if(_array.length < limit){
        setDepositError('Invalid Deposit Selection. Please choose at least '+limit+' NFTs, one from each');
        return false;
    }
...
const initContracts = async() => {
    ...
    let rows = [];
    for (let i = 0; i < _nftcombination; i++) {
     rows.push({count : 1}) ;
    }
    setCombination(rows);
...
<ModalFooter>
 ...
 <Modal isOpen={depositmodal} toggle={unstakeToggle}   centered={true}>
   ...
   <ModalBody>
     ...
     <label><br />Choose one from each NFT to stake
    ...
    <div className="" >
        
        {
            limit > 0 && combination.length > 0 && combination.map((v,i) => {
                return (
                    <ChoosNFt index={i} address={NFT_STAKE} handleDepositChange={() => handleDepositChange} />
